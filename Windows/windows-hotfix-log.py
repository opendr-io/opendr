...
class WindowsHotfixLogger(attr.LoggerParent):
    def __init__(self):
        ...
        self.interval: float = attr.get_config_value(
            "Windows", "HotfixInterval", 60.0, "float"
        )
    ...
    def fetch_hotfix_events(self) -> list[dict]:
        ...
        for hotfix in hotfix_data:
            hotfix["InstalledOn"] = (
                hotfix["InstalledOn"]["DateTime"]
                if isinstance(hotfix["InstalledOn"], dict)
                else hotfix["InstalledOn"]
            )
            hotfix["InstalledOn"] = (
                parse(hotfix["InstalledOn"]) if hotfix["InstalledOn"] else ""
            )
        ...
        hotfix_data = [
            {val: hotfix[key] for key, val in column_names.items()}
            for hotfix in hotfix_data
        ]

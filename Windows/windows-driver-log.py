...
class WindowsDriverLogger(attr.LoggerParent):
    def __init__(self):
        super().__init__()
        self.interval: float = attr.get_config_value(
            "Windows", "DriverInterval", 60.0, "float"
        )
    ...
    def fetch_defender_events(self) -> list[dict]:
        ...
        driver_data = [
            {val: driver[key] for key, val in column_names.items()}
            for driver in driver_data
        ]
        return driver_data
    ...
    def log_existing(self) -> None:
        ...
        for data in driver_data:
            self.prev_drivers.add(
                (data["driver_version"], data["device_id"], data["pdo"])
            )

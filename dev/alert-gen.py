...
class AlertGen:
    ...
    def populate_rules(self):
        with open(
            Path(__file__).parent.absolute() / "alertrules.csv", encoding="utf8"
        ) as csvfile:
    ...
    def search_log(
        self, directory_path: str, pattern: str, type: str = ""
    ) -> list[str]:
        matches = []
        ...
        if not directory.is_dir():
            print(
                f"[!] Not a directory: {directory}. attempting to check default folder"
            )
                ...
                for line in file:
                    ...
                    if timestamp_match:
                        log_time = datetime.strptime(
                            timestamp_match.group(0), self.time_format
                        )
                        if log_time >= self.search_interval and re.search(
    ...
    def augment_events(self) -> None:
        ...
        for rule in self.rules:
            matches = self.search_log(
                self.paths[rule["type"]], rule["pattern"], rule["type"]
            )
            if not matches:
                continue
        ...
        if alerts_generated:
            print(
                f"{alerts_generated} alerts were generated during the time period ({self.search_interval.strftime(self.time_format)} - {datetime.now().strftime(self.time_format)})."
            )
        else:
            print(
                f"No suspicious activity detected ({self.search_interval.strftime(self.time_format)} - {datetime.now().strftime(self.time_format)})."
            )

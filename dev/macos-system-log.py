...
class MacOSSystemLogger(attr.LoggerParent):
    def __init__(self):
        super().__init__()
        ...
        self.setup_logger()
        self.log_existing()
        print("LinuxSystemLogger Initialization complete")
        self.logger.write_debug_log(
            f"timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | "
            f"hostname: {self.hostname} | source: system | platform: macos | event: start "
        )
    ...
    def stop_logger(self) -> None:
        self.logger.write_debug_log(
            f"timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | "
            f"hostname: {self.hostname} | source: system | platform: macos | event: stop "
        )
    ...
    def monitor_events(self) -> None:
        ...
        if int(time.time()) % 10 == 0:
            self.logger.write_debug_log(f'timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | '
                                f'hostname: {self.hostname} | source: system | platform: macos | event: progress | '
                                f'message: {self.logger.log_line_count} log lines written | value: {self.logger.log_line_count}')

if __name__ == "__main__":
    system = MacOSSystemLogger()
    while True:

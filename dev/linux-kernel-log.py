...
class LinuxKernelLogger(attr.LoggerParent):
    def __init__(self):
        ...
        self.interval: float = attr.get_config_value(
            "Linux", "KernelInterval", 60.0, "float"
        )
    ...
    def get_kernel_taint_status(self):
        try:
            with open("/proc/sys/kernel/tainted", "r") as f:
                ...
                flags = [
                    self.TAINT_FLAGS[bit]
                    for bit in self.TAINT_FLAGS
                    if value & (1 << bit)
                ]
                return value, flags
        except Exception:
    ...
    def get_module_info(module: str):
        try:
            result = subprocess.run(
                ["modinfo", module], capture_output=True, text=True, check=True
            )
    ...
    def log_existing(self) -> None:
        ...
        taint_info: str = (
            f"tainted: {taint_val} | "
            + " | ".join([f"{flag}=True" for flag in taint_flags])
            if taint_flags
            else "tainted: 0 | clean_kernel=True"
        )

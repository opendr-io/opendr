...
class WindowsAutorunLogger(attr.LoggerParent):
    def __init__(self):
        super().__init__()
        self.interval: float = attr.get_config_value(
            "Windows", "AutorunInterval", 60.0, "float"
        )
        ...
        self.logger.write_debug_log(
            ...
            f"hostname: {self.hostname} | source: autorun | platform: windows | event: start "
        )

    def setup_logger(self) -> None:
        log_directory: str = (
            "tmp-windows-autoruns"
        ...
        self.logger: LoggingModule = LoggingModule(
            log_directory, ready_directory, "AutorunsMonitor", "autoruns"
        )

    def stop_logger(self) -> None:
        self.logger.write_debug_log(
            f"timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | "
            f"hostname: {self.hostname} | source: autorun | platform: windows | event: stop "
        )

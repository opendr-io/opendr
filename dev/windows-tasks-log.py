...
class WindowsTasksLogger(attr.LoggerParent):
    def __init__(self):
        ...
        self.interval: float = attr.get_config_value(
            "Windows", "TaskInterval", 60.0, "float"
        )
        self.logger = None
        ...
        self.logger.write_debug_log(
            ...
            f"hostname: {self.hostname} | source: tasks | platform: windows | event: start "
        )
    ...
    def setup_logger(self) -> None:
        ...
        self.logger: LoggingModule = LoggingModule(
            log_directory, ready_directory, "TaskMonitor", "scheduled_task"
        )

    def stop_logger(self) -> None:
        self.logger.write_debug_log(
            f"timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | "
            f"hostname: {self.hostname} | source: tasks | platform: windows | event: stop "
        )
    ...
    def fetch_scheduled_tasks(self) -> list[dict]:
        ...
        tasks = [
            {val: task[key] for key, val in column_names.items()} for task in tasks
        ]
        return tasks
    ...
    def log_existing(self) -> None:
        ...
        for data in task_data:
            if (
                ...
                continue
            self.prev_tasks.add(
                (data["task_name"], data["task_to_run"], data["start_time"])
            )

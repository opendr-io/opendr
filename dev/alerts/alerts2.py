...
# alert one search: python interpreter making internet connection
network_pattern = r"connection created.*process: python\.exe"
#alert two: python interpreter spawing a command shell
process_pattern = r"name:\s*cmd\.exe.*parent:\s*python\.exe"

# earch log files with a time filter
        ...
        with file_path.open("r", encoding="utf-8") as file:
            for line in file:
                ...
                if timestamp_match:
                    ...
                    if log_time >= one_hour_ago and re.search(pattern, line, re.IGNORECASE):
...
def send_notification(title, message):
    toaster.show_toast(title, message, duration=2)  # toast for 2 seconds

# process and print alerts
if network_matches:
    alert_title = "🚨 Python Internet Activity"
    alert_message = "python.exe created a connection in the last hour. Check logs."
    send_notification(alert_title, alert_message) 

    print("\n" + "="*50)
    print(f"{alert_title}\n{alert_message}")
    print("Matching log entries:\n")
    print("\n".join(network_matches))
    print("="*50)

if process_matches:
    alert_title = "⚠️ Python Shelled Out"
    alert_message = "python.exe ran cmd.exe in the last hour. Check logs."
    send_notification(alert_title, alert_message) 
...
if not (network_matches or process_matches):
    print(f"No suspicious activity detected in the last hour ({one_hour_ago.strftime(time_format)} - {now.strftime(time_format)}).")

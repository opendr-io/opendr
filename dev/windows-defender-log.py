...
class WindowsDefenderLogger(attr.LoggerParent):
    def __init__(self):
        ...
        self.interval: float = attr.get_config_value(
            "Windows", "DefenderInterval", 60.0, "float"
        )
        ...
        self.logger.write_debug_log(
            ...
            f"hostname: {self.hostname} | source: defender | platform: windows | event: start "
        )

    def setup_logger(self) -> None:
        log_directory: str = (
            "tmp-windows-defender"
        ...
        self.logger: LoggingModule = LoggingModule(
            log_directory, ready_directory, "DefenderMonitor", "defender"
        )

    def stop_logger(self) -> None:
        self.logger.write_debug_log(
            f"timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | "
            f"hostname: {self.hostname} | source: defender | platform: windows | event: stop "
        )
    ...
    def fetch_defender_events(self) -> list[dict]:
        ...
        for record in records:
            record["TimeCreated"] = (
                parse(record["TimeCreated"]) if record["TimeCreated"] else ""
            )
            record["event"] = "existing defender event"
            record["sid"] = self.sid

        records = [
            {val: record[key] for key, val in column_names.items()}
            for record in records
        ]
    ...
    def log_existing(self) -> None:
        ...
        for record in record_data:
            if (
                ...
                record.get("event_id", ""),
                record.get("title", ""),
            ) in self.prev_records:
                continue
            self.prev_records.add(
                (
                    record.get("timestamp", ""),
                    record.get("event_id", ""),
                    record.get("title", ""),
            ...
            self.logger.write_log(
                " | ".join([f"{key}: {record[key]}" for key in record])
            )

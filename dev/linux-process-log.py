...
class LinuxProcessLogger(attr.LoggerParent):
    def __init__(self):
        super().__init__()
        self.interval: float = attr.get_config_value(
            "Linux", "ProcessInterval", 0.1, "float"
        )
        self.previous_processes: set[int] = set()
        self.setup_logger()
        self.log_existing()
        print("LinuxProcessLogger Initialization complete")
        self.logger.write_debug_log(
            f"timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | "
            f"hostname: {self.hostname} | source: process | platform: linux | event: start "
        )
    ...
    def setup_logger(self) -> None:
        ...
        self.logger: LoggingModule = LoggingModule(
            log_directory, ready_directory, "ProcessMonitor", "process"
        )

    def stop_logger(self) -> None:
        self.logger.write_debug_log(
            f"timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | "
            f"hostname: {self.hostname} | source: process | platform: linux | event: stop "
        )
    ...
    def log_existing(self) -> None:
        ...
        for proc in psutil.process_iter(
            attrs=["pid", "name", "exe", "username", "cmdline"]
        ):
            try:
    ...
    def monitor_events(self) -> None:
        ...
        if int(time.time()) % 10 == 0:
            self.logger.write_debug_log(
                f"timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | "
                f"hostname: {self.hostname} | source: process | platform: linux | event: progress | "

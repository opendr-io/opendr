...
class NetworkAug:
    def __init__(self):
        self.interval = attr.get_config_value(
            "Augment", "NetworkAugInterval", 43200.0, "float"
        )
    ...
    def fetch_unique_ips(self):
        with psycopg.connect(
            ...
            with connection.cursor() as cursor:
                ips = cursor.execute(
                    f"SELECT DISTINCT(sourceip) FROM systemevents WHERE timestmp > '{(datetime.now() - timedelta(seconds=self.interval)).strftime('%Y-%m-%d %H:%M:%S')}'"
                ).fetchall()
            connection.close()
    ...
    def enrich_network(self, ip, dns_name, as_name) -> None:
        with psycopg.connect(
            ...
            with connection.cursor() as cursor:
                cursor.execute(
                    f"UPDATE systemevents SET dnsname = '{dns_name}', asname = '{as_name}' WHERE sourceip = '{ip}' AND timestmp > '{(datetime.now() - timedelta(seconds=self.interval)).strftime('%Y-%m-%d %H:%M:%S')}'"
                )
            connection.commit()

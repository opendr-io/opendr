...
class LoggingModule:
    def __init__(
        self, log_directory: str, ready_directory: str, logger_name: str, file_name: str
    ):
        ...
        self.debug_mode: bool = get_config_value(
            "General", "DebuggingMode", False, "bool"
        )
    ...
    def setup_logging(self, debug: bool = False) -> None:
        ...
        if not debug:
            log_filename: str = os.path.join(
                self.log_directory, f"{self.file_name}_{random_int}_{current_time}.log"
            )
            ...
            log_filename: str = os.path.join(
                self.debug_generator_directory,
                f"debug_{self.file_name}_{random_int}_{current_time}.log",
            )
            self.debug_logger: logging.Logger = logging.getLogger(
                "Debug" + self.logger_name
            )
    ...
    def check_logging_interval(self) -> None:
        ...
        if (
            current_interval != self.last_interval
        ):  # Rotate log file at the start of a new minute
            self.setup_logging()
            if self.debug_mode:
                self.setup_logging(debug=True)
